...
function coordsDentroDeDistrito(
  lat: number,
  lng: number,
  distritoNome: string
): boolean {
...
export default function MapView({
  ...
  const lastSelectTsRef = useRef(0);
  ...
  function shouldIgnoreDuplicateSelect() {
    const now = Date.now();
    if (now - lastSelectTsRef.current < 350) return true;
    lastSelectTsRef.current = now;
    return false;
  }
      ...
      fetchGeoJSON(DISTRITOS_URL).then((geojson) => {
        ...
        distritosRef.current = L.geoJSON(geojson, {
          ...
          onEachFeature(feature: any, layer: any) {
            ...
            const handleDistritoSelect = (e?: any) => {
              if (shouldIgnoreDuplicateSelect()) return;
            ...
            layer.on("click", handleDistritoSelect);
            layer.on("touchend", handleDistritoSelect);
            layer.on("pointerup", handleDistritoSelect);
          },
        }).addTo(map);
      ...
      fetchGeoJSON(MUNICIPIOS_URL).then((geojson) => {
        ...
        municipiosRef.current = L.geoJSON(geojson, {
          ...
          onEachFeature(feature: any, layer: any) {
            ...
            const handleMunicipioSelect = (e?: any) => {
              if (shouldIgnoreDuplicateSelect()) return;
              ...
              if (distritoId && conNome) {
                cbConcelho.current?.(distritoId, conNome);
              }
            };

            layer.on("click", handleMunicipioSelect);
            layer.on("touchend", handleMunicipioSelect);
            layer.on("pointerup", handleMunicipioSelect);
          },
        });
    ...
    const tryAdd = (retries = 20) => {
      ...
      (async () => {
        ...
        postos.forEach((posto) => {
          ...
          if (
            posto.distrito &&
            !coordsDentroDeDistrito(posto.lat, posto.lng, posto.distrito)
          ) {
            return;
          }
          ...
          const combsHtml =
            ...
            <div style="display:flex;justify-content:space-between;gap:1rem;font-size:0.72rem">
              ...
              .join("") ||
            `<span style="font-size:0.72rem;color:#888">Sem preços</span>`;
  ...
  <a href="${
    posto.lat && posto.lng
      ...
      : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
          [posto.nome, posto.morada, posto.localidade]
            .filter(Boolean)
            .join(", ")
        )}`
